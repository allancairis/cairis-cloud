        ...
        <div className="grid gap-4">
          {initiatives.map((project, index) => (
            <Reveal key={project.title} delay={index * 0.04}>
              <article className="glass grid gap-4 rounded-lg p-5 md:grid-cols-[1fr_auto] md:items-center">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.16em] text-cyan">{project.signal}</p>
                ...
                <span className="hidden h-px w-24 bg-gradient-to-r from-cyan to-transparent md:block" aria-hidden="true" />
              </article>
            </Reveal>
          ))}
        </div>

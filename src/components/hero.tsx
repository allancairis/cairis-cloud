...
export function Hero() {
  return (
    <section id="topo" className="relative isolate min-h-screen overflow-hidden pt-28" aria-labelledby="hero-title">
      <Image src="/assets/hero-architecture-ai.svg" alt="" fill priority sizes="100vw" className="absolute inset-0 -z-20 object-cover opacity-40" />
      ...
      <div className="section-shell grid min-h-[calc(100vh-7rem)] items-center gap-12 pb-14 lg:grid-cols-[minmax(0,1.08fr)_minmax(360px,.74fr)]">
        <div className="max-w-4xl">
          ...
          <motion.p className="body-lg mt-8 max-w-2xl" initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.16 }}>Eu conecto liderança técnica, engenharia hands-on e visão de negócio para transformar complexidade tecnológica em vantagem estratégica.</motion.p>
          ...
          <motion.dl className="mt-12 grid max-w-3xl gap-3 sm:grid-cols-3" initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.32 }}>{stats.map((stat) => <div key={stat.label} className="glass rounded-lg p-4"><dt className="text-xs uppercase text-muted">{stat.label}</dt><dd className="mt-3 font-display text-3xl font-bold text-white">{stat.value}</dd></div>)}</motion.dl>
        </div>
        <motion.aside className="glass relative overflow-hidden rounded-lg p-5" initial={{ opacity: 0, x: 34 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9, delay: 0.18 }} aria-label="Painel executivo">
          ...
          <div className="relative flex items-start justify-between gap-5 border-b border-white/10 pb-5"><div><p className="text-sm text-muted">Executive technology profile</p><h2 className="mt-2 font-display text-3xl font-semibold text-white">Allan Cairis</h2><p className="mt-2 text-sm text-cyan">Arquitetura corporativa | DevSecOps | IA aplicada</p></div><div className="grid h-14 w-14 shrink-0 place-items-center rounded-md border border-cyan/30 bg-cyan/10"><ShieldCheck className="h-7 w-7 text-cyan" aria-hidden="true" /></div></div>

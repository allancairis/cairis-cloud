...
export function Header() {
  return (
    <motion.header
      ...
      className="fixed left-0 right-0 top-0 z-40 border-b border-white/10 bg-carbon/72 backdrop-blur-2xl"
    ...
    >
      <div className="section-shell flex min-h-20 items-center justify-between gap-5">
        <a href="#topo" className="focus-ring flex items-center gap-3 rounded-md">
          <span className="grid h-9 w-9 place-items-center rounded-md border border-white/15 bg-white/8 font-display text-sm font-bold text-cyan shadow-glow">
            AC
          </span>
        ...
        <nav className="hidden items-center gap-7 text-sm text-muted md:flex" aria-label="Secoes">
          {navItems.map((item) => (
            <a key={item.href} className="focus-ring rounded-md transition hover:text-white" href={item.href}>
              {item.label}
            </a>
          ))}
        </nav>
